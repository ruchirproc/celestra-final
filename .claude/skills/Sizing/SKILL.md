...
- **Only Tier 1 and Tier 2 are ever field-relevant in this skill.** `Tier 1 Calls/HCP/year = 12` and `Tier 2 Calls/HCP/year = 6` are fixed constants — never derive, research, or override them. `Tier 3`, `Tier 4`, and every tier beyond `Tier 2` are always irrelevant tiers: `Reach % = 0` and `Calls/HCP/year = 0` in every view, no matter what reach/relevance signal appears in the source file for those tiers, and no matter what any other tier-relevance or "lowest-priority tier" logic elsewhere in this document might otherwise suggest. This rule is a fixed business constraint, not a data-driven inference, and it overrides all other tiering logic in this skill. Under this rule, Tier 2 is always the lowest-priority tier — never Tier 3 or Tier 4.
- Zero out irrelevant tiers in every model view. After tier normalization, any tier identified as irrelevant for field-based engagement must receive `Reach % = 0` and `Calls/HCP/year = 0` in `Normal_Workload`, all reach scenarios, and all frequency scenarios, and must not be considered in the sizing exercise. Keep those tiers visible for transparency, but they must contribute 0 effective HCPs, 0 calls, 0 Effective FTEs, and 0 FT/HCP. **The lowest-priority tier is defined as the field-relevant tier with the lowest non-zero reach %. A tier with 0% reach is an irrelevant tier, never the lowest-priority tier. The lowest-priority tier keeps its non-zero reach and calls/HCP/year and remains fully included in sizing; never zero it out simply for being lowest priority. In this skill, since only Tier 1 and Tier 2 are ever field-relevant, Tier 2 is always the lowest-priority tier.**
...
2. **Lowest-priority tier:** The field-relevant tier with the lowest non-zero reach % after applying segment relevance logic and irrelevant-tier zeroing. Tiers at 0% reach are never the lowest-priority tier — they are irrelevant tiers. Since Tier 3, Tier 4, and any tier beyond Tier 2 are always irrelevant in this skill (0% reach by fixed rule, not by inference), Tier 2 is always the lowest-priority tier — it is never Tier 3 or Tier 4. The lowest-priority tier keeps its non-zero reach and calls/HCP/year and remains fully included in sizing.
...
- Generate defaults for all tiers present in the data, not only Tier 1 and Tier 2 — except Tier 3, Tier 4, and any tier beyond Tier 2, which are always irrelevant per the fixed rule above and always default to 0 reach and 0 calls, never a data-derived non-zero value.
- Taper reach and calls by tier. Never add or drop tiers without basis.
- Force the irrelevant tier(s) to `Reach % = 0` and `Calls/HCP/year = 0` after all tier defaults, scenario levers, and user/client overrides are considered, and exclude them from the sizing exercise. This is a compulsory exclusion rule for sizing. **Do not zero out the lowest-priority tier. The lowest-priority tier is defined as the field-relevant tier with the lowest non-zero reach % — tiers at 0% reach are irrelevant tiers, not the lowest-priority tier. The lowest-priority tier keeps its non-zero reach and calls and remains in sizing. If the user/client wants workload for a group previously flagged as irrelevant, reclassify that group as field relevant, assign a defensible non-zero reach, and document the rationale in `Diagnostics_QA` — this reclassification escape hatch never applies to Tier 3, Tier 4, or any tier beyond Tier 2, which stay locked at 0 reach and 0 calls regardless of data signals or inferred client intent.**
